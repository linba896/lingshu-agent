...
class CredentialVault:
    ...
    def _save(self) -> None:
        ...
        with self._lock:
            if self._master_password:
                # Derive key from password before encrypting
...
class InputSanitizer:
    ...
    def sanitize_shell(self, text: str) -> str:
    ...
    def sanitize_path(self, text: str) -> str:
        """Sanitize path, return empty string if unsafe"""
        if not text:
            return ""
        if self.__class__.PATH_TRAVERSAL.search(text):
            return ""
    ...
    def sanitize(self, text: str, type: str = "generic") -> str:
        """Sanitize input text by type"""
        if type == "sql":
            return self.sanitize_sql(text)
        elif type == "html":
            return self.sanitize_html(text)
        elif type == "shell":
            return self.sanitize_shell(text)
        elif type == "path":
            return self.sanitize_path(text)
        else:
            return text
...
class SecurityManager:
    ...
    def sign_data(self, data: str, context: str = "") -> str:
        """Sign data string"""
        return self.signature.sign(data.encode())
    
    def verify_signature(self, data: str, signature: str, context: str = "") -> bool:
        """Verify data signature"""
        return self.signature.verify(data.encode(), signature)
    ...
    def verify_password(self, password: str, hash_val: str) -> bool:
        ...
        return self.crypto.verify_password(password, hash_val)
    
    def store_credential(self, key: str, value: str, metadata: Optional[Dict[str, Any]] = None) -> None:
        """Store credential with optional metadata"""
        self.vault.store(key, value, metadata=metadata)
        self.audit.log("CREDENTIAL_STORE", {"key": key}, True)
    
    def log_audit_event(self, event: str, data: Dict[str, Any]) -> None:
        """Log security audit event"""
        self.audit.log(event, data, True)
