...
# -*- coding: utf-8 -*-
"""
灵枢（LingShu）Agent — 视觉理解模块（Phase 4）
功能：屏幕截图 + VLM 解析 + GUI 元素定位 + 视觉任务理解

核心流程：
  1. 屏幕捕获（mss / Pillow 双备选）
  2. 图像预处理（缩放、编码 base64、分辨率自适应）
  3. VLM 推理（Qwen3-VL-8B-Instruct / 轻量 fallback）
  4. 视觉解析（UI 元素定位、OCR 文本、语义描述）
  5. 多模态联动（语音指令 "打开这个" → 截图 → 识别 → 执行）

依赖：
  - mss         → 高性能屏幕捕获（跨平台）
  - Pillow      → 图像处理（备选截图）
  - transformers + torch → VLM 模型加载
  - qwen-vl-utils → Qwen VL 工具函数
"""
